...
{
    /// <summary>
    /// Protobuf serializer for cluster messages to Interop to Akka (Java/Scala)
    /// </summary>
  internal class InteropClusterMessageSerializer : SerializerWithStringManifest
  {
    ...
    public override string Manifest(object o)
    {
      ...
      return type.AssemblyQualifiedName != null && classDictionary.ContainsKey(type.AssemblyQualifiedName)
        ? classDictionary[type.AssemblyQualifiedName]
        : type.AssemblyQualifiedName;
    }


    /// <summary>
    /// Internal Dictionary to track Assembly Names and Java Class names.
    /// </summary>
    private Dictionary<string, string> _classDictionary;


    // ReSharper disable once MemberCanBePrivate.Global
    public Dictionary<string, string> classDictionary
    {
      get
      {
    ...
    public InteropClusterMessageSerializer(ExtendedActorSystem system)
        : base(system)
    {
    ...
    {
      if (!manifest.Contains("PublicKeyToken"))
      {
        // Not a .Net Assembly Name, must be java.  Lets look it up.
        if (classDictionary.Any(c => c.Value == manifest))
        {
          var dotNetName = classDictionary.FirstOrDefault(c => c.Value == manifest).Key;
          manifest = dotNetName;
        }
      }
      ...
      if (type == null) throw new InvalidMessageException($"Message of manifest type {manifest} could not be loaded.");
    ...
    /// <summary>
    /// Using reflection, this iterates through the referenced libraries, and looks for all <see cref="JavaClassname"/> attributes, and caches them.
    /// </summary>
    private void buildClassDictionary()
    {
      _classDictionary = new Dictionary<string, string>();
      ...
      foreach (var a in asms)
      {
        try
        {
          var asm = Assembly.Load(a.ToString());
          scanAssembly(asm);
          var a2 = asm.GetReferencedAssemblies();
          foreach (var a2A in a2)
          {
            try
            {
              var asm2 = Assembly.Load(a2A.ToString());
              scanAssembly(asm2);
            }
            catch
            {
              // Some types can not be loaded and iterated, so continue
            }
          }
        }
        catch
        {
          // Some Assemblies can not be loaded and iterated, so continue
        }
    ...
    /// </summary>
    /// <param name="a"></param>
    private void scanAssembly(Assembly a)
    {
      foreach (var type in a.GetTypes())
      {

        if (type.GetCustomAttributes(typeof(InteropManifest), true).Length <= 0) continue;

        var javaClassName =
          ((InteropManifest)Attribute.GetCustomAttribute(type, typeof(InteropManifest))).Name;

        var dotNameName = type.AssemblyQualifiedName;

        if (dotNameName != null && !classDictionary.ContainsKey(dotNameName))
        {
          _classDictionary.Add(dotNameName, javaClassName);
        }
      }
    }
